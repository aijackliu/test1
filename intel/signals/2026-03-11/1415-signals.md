# Signal Engine Scan — 2026-03-11 14:15 Asia/Taipei

## Scan Status
- Prompt: `/Users/claireye/clawd/prompts/intel/signal-engine.prompt.md`
- Run time: 2026-03-11 14:15 Asia/Taipei
...
- 熱度分：87/100
- 等級：L1
- 來源：Bloomberg.com, CNBC, The Information
- 命中關鍵字：oracle, openai, data center, stargate, nvidia, debt
- 摘要（2-3 句）：本輪抓取 8 則 headline，跨來源 8。主要敘事：Oracle and OpenAI End Plans to Expand Flagship Data Center - Bloomberg.com / Oracle is building yesterday’s data centers with tomorrow’s debt - CNBC。
...
- 摘要（2-3 句）：本輪抓取 8 則 headline，跨來源 7。主要敘事：Bitcoin manipulation claims face pushback as ETFs snap 5-week outflow run: Finance Redefined - TradingView / The year in data: 5 charts that show how crypto changed in 2025 - The Block。
- 判讀（1-2 句）：RSS headline 初判，需高權重來源補強。
- 反向驗證（反證/待確認項）：缺 X 即時流與互動增速。
- 建議動作（Follow-up）：下一輪補官方來源與財報/公告。
...
- 摘要（2-3 句）：本輪抓取 8 則 headline，跨來源 6。主要敘事：CPI data, oil inventories among economic data due Wednesday By Investing.com - Investing.com Australia / WTI sees sharp spike as Middle East tensions continue to sizzle - FXStreet。
- 判讀（1-2 句）：RSS headline 初判，需高權重來源補強。
- 反向驗證（反證/待確認項）：缺 X 即時流與互動增速。
- 建議動作（Follow-up）：下一輪補官方來源與財報/公告。
...
- 熱度分：70/100
- 等級：L2
- 來源：Reuters, WTVB, Bloomberg.com
- 命中關鍵字：pboc, stimulus, yuan, loan, credit
- 摘要（2-3 句）：本輪抓取 8 則 headline，跨來源 7。主要敘事：China's November new loans miss forecast as housing slump persists - Reuters / China’s December new bank loans beat forecast as stimulus juices credit demand - WTVB。
- 判讀（1-2 句）：RSS headline 初判，需高權重來源補強。
- 反向驗證（反證/待確認項）：缺 X 即時流與互動增速。
- 建議動作（Follow-up）：下一輪補官方來源與財報/公告。
